...
# cursor.execute("""DELETE FROM [XXXX_Email].[dbo].[tblQuoteStage]""")

# #Code to reset the Identity Value (Auto-Increment)
# cursor.execute("""DBCC CHECKIDENT ('[XXXX_Email].[dbo].[tblQuoteStage]', RESEED, 0)""")
...
email_sender = outlook.Session.Accounts['pravin.subramanian@company.com']

for i in (range(len(outlook.Folders))):
    #Condition to take emails from that particular account.
    if (outlook.Folders.Item(i+1).Name == 'autoenquiries.region@company.com'):
            ...
            if (folder.Name == 'Inbox'):
                ...
                for message in messages:
                    if message.UnRead == True:
                        ...
                        mail.To = 'pravin.subramanian@company.com; pravin.subramanian@company.com' #Recipients list
                        ...
                        if  ('noreply@microsoft' not in email_sender):
                            msg_no+=1
                            body = message.Body
                            ...
                            try:
                                ...
                                body_no_ship_sign = re.sub(r'(?:Please do not delete this email\s*.*shipserv.com>  .)', '', body_with_no_sign, flags = re.IGNORECASE|re.DOTALL|re.MULTILINE)
...
#************************************************Part 3***********************************************************************************************************#                                
                                ...
                                if ('IMO NO.' in (text_ent)):
                                    if (len(text_ent['IMO NO.'])) <=4:
                                        text_ent = {key: val for key, val in text_ent.items() if key != 'IMO NO.'}
                                ...
                                if 'CUST' in text_ent:
                                    ...
                                    if ('IMO NO.' in text_ent) and ('VESS' in text_ent) and ((len(imo_data))==0) and ((len(df_final_rev))==0):
                                        ...
                                        cursor.execute(""" 
                                        insert into [XXXX_Email].[dbo].[tblVessel] (fldVesselID,fldVIMO,fldVName,fldCustomerID,MergedOn,MergedBy) 
                                        Select ?,?,?,?,?,?
                                        Where not exists(select fldVIMO,fldVName,fldCustomerID from [XXXX_Email].[dbo].[tblVessel]
                                        where fldVIMO like (?) and fldVName like (?) and fldCustomerID like (?))
                                        """,(int(vessID_serial['max_vessid'][0])+1),text_ent.get('IMO NO.'),'NA',str(cust_rev['fldCustomerID'][0]),str(dt),'PRAVIN SUBRAMANIAN',text_ent.get('IMO NO.'),'NA',str(cust_rev['fldCustomerID'][0]))
                                        conn_str.commit()
...
#********************************************End of Final Joins***************************************************************************************************************#                                                            
                                ...
                                if (len(Cust_quote))>0:
                                    ...
                                    """,str(Cust_quote['fldCustomerID'][0]),str(Cust_quote['fldVesselID'][0]),str(Cust_quote['fldCustContactID'][0]),str(dt),'SA',str(Cust_quote['fldCustomerID'][0])
                                    ...
                                    <td> <span style="font-size:14px;font-family: Calibri;float:left;"> A new quote has been prepared by {creatBy} in the MSMS DB with the following details: </span> </td>
                                    </head>
                                    <body>
                                    <td></td><br><br>
                                    ...
                                    <br><br><br><br><td><span style="font-size:15px;font-family: Calibri;float:left;">Please Open Database to view quote.
